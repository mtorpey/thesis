...
$\phi : \PT_n \to \Prt_n$ be the map that sends a partial transformation $\tau$
to the bipartition $\left\{\big(a, (a\tau)'\big) : a \in \dom \tau\right\}^e$.
It is easy to see that $\phi$ is a monomorphism.  Hence $\Prt_n$ contains a copy
of $\PT_n$, and therefore it also contains copies of the full transformation
monoid $\T_n$, the symmetric inverse monoid $\I_n$, and the symmetric group
$\Sym_n$.  Note that $\Prt_n$ is a regular semigroup, with an inverse of
$\alpha \in \Prt_n$ given by $\alpha^\star$.
...
\begin{proposition}
  ...
  \end{enumerate}
  \begin{proof}
    Parts (i) to (iv) are from \cite{fitzgerald_2011}, and part (v) follows
    naturally from (iii) and (iv).
  \end{proof}
\end{proposition}
