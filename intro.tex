...
These are formally defined as a class of equivalence relations under an
interesting composition operation, but they can also be seen as a generalisation
of partial transformations.  We begin with the formal definition.
...
\begin{example}
  ...
  bipartition of degree $5$ with blocks $\{1, 2, 5, 1', 2'\}$, $\{4\}$,
  $\{3, 4', 5'\}$ and $\{3'\}$.  The diagrams of these two figures, along with
  their product $\alpha \beta$, are shown in Figure
  \ref{fig:bipartition-example}.  Note that two different diagrams are shown for
  $\alpha \beta$.
  \begin{figure}[h]
    \centering
...
appropriate kernel block written above the corresponding cokernel block.
...
that $(\alpha^\star)^\star = \alpha$.  Hence it can act as a semigroup inverse
function for bipartitions.
...
the partial transformation monoid $\PT_n$ embeds as follows: let
$\phi : \PT_n \to \Prt_n$ be defined such that, for a partial transformation
$\tau$, the partition $(\tau)\phi$ has a block $\{a, (a\tau)'\}$ for each $a$ in
the domain of $\tau$, and all other blocks singletons.  It is easy to see that
$\phi$ is a monomorphism.  Hence $\Prt_n$ contains a copy of $\PT_n$, and
therefore copies of the full transformation monoid $\T_n$, the symmetric inverse
monoid $\I_n$, and the symmetric group $\Sym_n$.  Note that $\Prt_n$ is a
regular semigroup, with an inverse of $\alpha \in \Prt_n$ given by
$\alpha^\star$.
...
relations and the containment of $\JJ$-classes very simply in terms of domain,
codomain, kernel, cokernel, and rank.  The result is taken from
\cite{fitzgerald_2011}.
...
the Bell number $B_{2n}$ \cite[A000110]{oeis}.  It therefore grows much faster
than the symmetric group $\Sym_n$ with $n!$ elements, the symmetric inverse
monoid $\I_n$ with $\sum_{i=0}^n \binom{n}{i} \frac{n!}{i!}$ elements, and even
the full transformation monoid $\T_n$ with $n^n$ elements.  The degree of this
difference is illustrated in Table \ref{tab:pn-size}.
