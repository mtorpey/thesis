...
methods which they can run.  For completeness, we present one such formulation.
...
\begin{definition}
  ...
  \item $\delta: Q \times \Sigma \to
    ...
    computation;
  \item $H \subseteq Q$ is a set states known as \textit{halting states}.
  \end{itemize}
  This machine is paired with a conceptual infinite \textit{tape}---an infinite
  ...
  $Q \times \Sigma \times \{L,R\}$: it then changes state from $q$ to $q'$,
  replaces the character $\sigma$ on the tape with $\sigma'$, and moves the
  read/write head one character left or right according to $d$.  This process is
