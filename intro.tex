...
\begin{example}
  ...
  same element of $S$ if and only if they have the same number of occurrences of
  $a$ and the same number of occurrences of $b$.
\end{example}

In the preceding example, there is an algorithm to decide the word problem; we
...
their word problem.  Consider the
following example from Gennadi\'{i} Makanin with only three generators.
...
% TODOSOON: these two lines
Might be hard to say 

A more full description is here, but it's not the point of my thesis.
