...
\cite{ourpaper}, and are included in this thesis with the permission of my
co-authors.
...
a reflexive pair and lies in the congruence $\sigma$.  If $(ax,bx) \notin
\Delta_S$ then $(ax,bx) \in \sigma$.  Hence, either way, $(ax,bx)$ is in
...
\begin{proposition}
  ...
  \begin{proof}
    ...
    know that the elements in $I_0$ are those that have rank $0$, and therefore
    all have empty codomains; and we know
    from Proposition \ref{prop:mn-greens}(ii) that elements are $\lL$-related if
    and only if they share a codomain and cokernel, so we have the statement for $\lambda_0$.
    ...
    $(\widehat\alpha, \widehat\beta) \in \lL^{I_0}$.
    All elements of $I_0$ have empty codomain, so a pair satisfies this if
    and only if $\widehat\alpha$ and $\widehat\beta$ have the same cokernel.
    Since $\alpha$ and $\widehat\alpha$ share a cokernel, and $\beta$ and
    $\widehat\beta$ share a cokernel, this is therefore satisfied if and only if
    $\alpha$ and $\beta$ share a cokernel, and so we have the statement for
...
$n = 1$ then $\Mot_n$ has only $2$ elements, and its only congruences are
$\Delta_{\Mot_n}$ and $\nabla_{\Mot_n}$.
...
\begin{theorem}
  ...
  \item The congruences of $\Mot_n$ are precisely
    ...
    \ldots, R_n\}$;
  \item The congruence lattice of $\Mot_n$ is as shown in Figure
...
relations in $\Gamma$ are congruences has already been established.
Next we consider the joins of these congruences in Lemmas
...
that $\Gamma$ is closed under taking joins.
Then, in order to
...
congruences.
Since any congruence is a join of
...
proof of Theorem \ref{thm:mn-congs}.  Firstly, we will focus on meets and
joins of congruences, and show that there are no other congruences that can be
generated by taking meets and joins of congruences in $\Gamma$.

% HERE ARE THE TWO MEET/JOIN LEMMAS
\pagebreak
\begin{lemma}
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    observe that the other part follows by a similar argument.  Since
  ...
  \end{proof}
\end{lemma}
% END OF THE TWO MEET/JOIN LEMMAS.

The last two lemmas together prove that the congruences in $\Gamma$ form the
lattice shown in Figure \ref{fig:mn-cong-lattice} with respect to containment.
Now we only need to show that there are no other principal congruences on
$\Mot_n$, and the proof of Theorem \ref{thm:mn-congs} will be complete.  We will
do this by considering the generating pairs of all the congruences in $\Gamma$,
and showing that any pair in $\Mot_n \times \Mot_n$ generates one of them.  The
results are summarised in Table \ref{tab:mn-genpairs}.

% NEXT: THE GEN PAIRS LEMMAS
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    by induction on $r$, the number of kernel-classes of $\gamma$: if
    ...
    $(\alpha,\beta) \in \lambda_0 \setminus \Delta$, Proposition
    ...
    Since $\sigma$ is left- and right-compatible, we deduce that
    $\gamma = \tau\alpha\gamma$ is $\sigma$-related to $\tau\beta\gamma$.  Hence $\gamma$
    ...
    generality, additional labelled elements are assumed to be from $A_1$ or $B_1$:
    ...
    \item $\beta = \bipart{c|c|c|c|c}{2-5}
      ...
      so that $\beta$ is the same as $\alpha$ but with $k$ in the transversal
      instead of $i$;
    \item $\beta = \bipart{c|c|c|c|c}{2-5}
      {i & A_1 & A_2 & \cdots & A_r}{l & j & B_2 & \cdots & B_s}$,
      so that $\beta$ is the same as $\alpha$ but with $l'$ in the transversal
      instead of $j'$;
    \item $\beta = \bipart{c|c|c|c|c}{2-5}
      {k & i & A_2 & \cdots & A_r}{l & j & B_2 & \cdots & B_s}$,
      so that $\beta$ is the same as $\alpha$ but with transversal $\{k, l'\}$
      instead of $\{i, j'\}$.
    \end{enumerate}
    Now, for any $a,b \in \bn$, let us denote by $\tau_{ab}$ the bipartition
    $(a, b')^e \in \Mot_n$ -- this has just one non-trivial block, $\{a,b'\}$.
    Let us use $\tau_\varnothing$ to denote the bipartition in $\Mot_n$
    consisting entirely of singletons.
    We will use the bipartitions $\tau_{ii}$ and $\tau_{jj}$. In all four cases
    above, we have
    $\tau_{ii}\alpha\tau_{jj}= \tau_{ij}$ and
    $\tau_{ii}\beta\tau_{jj}= \tau_\varnothing$.
    Since $\alpha \mathrel\sigma \beta$, we also have
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    obvious; we just need to consider the right-to-left implications.  First we
    ...
    the premise.  Clearly $(\alpha,\beta)^\sharp \subseteq \lambda_1$.
    To be outside $\lambda_0$, either $\alpha$ or $\beta$ must
    ...
    {j & B_1 & \cdots & B_s}{j & B_1 & \cdots & B_s}$, so that
    $\coker(\alpha\gamma) = \coker(\beta\gamma)$, and hence
    $(\alpha\gamma, \beta\gamma) \in \lambda_0$.
    In particular,
    ...
    \ref{lem:mn-lattice-inter}, we need only show that $\delta_1
    \subseteq (\alpha,\beta)^\sharp$, and (i) is complete.
    To do this, we will consider the cases (a)--(e) separately.

    Firstly, assume (a) holds.  We know that $\alpha \alpha^\star \alpha = \alpha$,
    and we can see that $\alpha\alpha^\star\beta = \widehat\alpha$.  Hence Lemma
    \ref{lem:low-genpairs}(iii) gives
    $\delta_1 = (\alpha, \widehat\alpha)^\sharp = (\alpha\alpha^\star\alpha,
    \alpha\alpha^\star\beta)^\sharp \subseteq (\alpha,\beta)^\sharp$, so
    ...
    {i & A_1 & A_2 & \cdots & A_r}{l & j & B_2 & \cdots & B_s}$,
    so that $\tau\alpha$ and $\tau\beta$ have the same kernel and cokernel but a
    different transversal.
    ...
    $\delta_1 \subseteq (\alpha, \beta)^\sharp$.  Hence
    $\lambda_1 \subseteq (\alpha, \beta)^\sharp$, and the proof of
    ...
    $\beta=\bipart{c|c|c}{1-3}{C_1 & \cdots & C_t}{D_1 & \cdots & D_u}$, noting
    that, since $(\alpha, \beta) \notin \rho_0$, we must have
    $\{A_1, \ldots, A_r\} \neq \{C_1, \ldots, C_t\}$.
    % noting that $\ker(\alpha)\not=\ker(\beta)$.
    % $\coker(\alpha)\not=\coker(\beta)$.
    By Lemma \ref{lem:mn-lattice-intra}, we have
    ...
    $R_0 \subseteq (\alpha,\beta)^\sharp$.  It is obvious that
    ...
    $\lambda_1$ nor $\rho_1$, we also know that $\ker\alpha \neq \ker\beta$ and
    $\coker\alpha \neq \coker\beta$.
    Hence we can write
    $\alpha = \bipart{c|c|c|c}{2-4}
    ...
    {k & C_1 & \cdots & C_t}{l & D_1 & \cdots & D_u}$, with
    $\big\{\{i\}, A_1, \ldots, A_r\big\} \neq \big\{\{k\}, C_1, \ldots, C_t\big\}$.
    Now, as in (iii),
    since $R_1 = \lambda_1 \vee \rho_1$, by Lemma \ref{lem:mn-lattice-intra}, we
    prove that $R_1 \subseteq (\alpha,\beta)^\sharp$ by showing that
    $(\alpha,\beta)^\sharp$ contains $\lambda_1$ and $\rho_1$.  We will prove
    the statement for $\lambda_1$, and observe that $\rho_1$ follows by a
    similar argument.  Let us proceed by
    ...
    {k & C_1 & \cdots & C_t}{j & B_1 & \cdots & B_s}$,
    and in case (g) we have
    $\beta\gamma = \bipart{c|c|c|c}{2-4}
    {k & C_1 & \cdots & C_t}{j & B_1 & \cdots & B_s}$.
    In either case, we have
    $(\alpha\gamma, \beta\gamma) \in
    \lambda_1 \setminus (\lambda_0 \cup \delta_1)$,
    so by (i), we have
    ...
    $(\alpha\tau, \beta\tau) \in
    \lambda_1 \setminus (\lambda_0 \cup \delta_1)$,
    so again by (i), we have
    $\lambda_1 = (\alpha\tau, \beta\tau)^\sharp
    \subseteq (\alpha,\beta)^\sharp$, completing this case.
    This completes the proof that
    $\lambda_1 \subseteq (\alpha,\beta)^\sharp$, and the proof that
    $\rho_1 \subseteq (\alpha,\beta)^\sharp$ is similar.  Hence
    $\lambda_1 \vee \rho_1 = R_1 \subseteq (\alpha,\beta)^\sharp$.  It is obvious that
    $(\alpha, \beta)^\sharp \subseteq R_1$, and so the proof of (iv) is
    complete.
  \end{proof}
\end{lemma}
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    If $r = 0$, then $\gamma = \widehat\gamma$ and the claim is satisfied;
    hence let us assume $r \geq 1$.
    ...
    $\bn \setminus \{i_1, \ldots, i_r\} = \{a_1, \ldots, a_{n-r}\}$ and
    $\bn \setminus \{j_1, \ldots, j_r\} = \{b_1, \ldots, b_{n-r}\}$.
    ...
    % TODO? what if r < x? trivial, but annoying notationally
    ...
    $\{c_2, a_2'\}$, upper (non-transversal) blocks $C_2, \ldots, C_t$, and the rest singletons.
    ...
    $D_1' = \{d_1', d_2'\}$ with $d_1 < d_2$.  Now choose two indices $p,q \in
    \{1, \ldots, k\}$ such that $i_p < i_q < a_1 < a_2$ (or, if this is
    impossible, such that $a_1 < a_2 < i_p < i_q$ or $i_q < a_1 < a_2 < i_p$).
    Let $\tau$ be the bipartition of rank $0$ with the upper blocks of $\gamma$
    and the lower blocks $\{i_p', a_2'\}$, $\{i_q', a_1'\}$, and the rest
    singletons.  Let $\kappa$ be the bipartition with $2$ transversals
    $\{j_p, d_1'\}$ and $\{j_q, d_2'\}$, lower blocks $D_2', \ldots, D_u'$, and
...
We have now classified the generating pairs of all the congruences in $\Gamma$,
and we find that this exhausts all the pairs in $\Mot_n \times \Mot_n$, as
summarised in Table \ref{tab:mn-genpairs}.  This proves that $\Gamma$ includes
all the principal congruences on $\Mot_n$, and since we know from Lemmas
\ref{lem:mn-lattice-intra} and \ref{lem:mn-lattice-inter} that these are closed
under taking joins, we can conclude that there are no other congruences on
$\Mot_n$.  Hence the congruence lattice in Figure \ref{fig:mn-cong-lattice} is
complete, and we have completed the proof of Theorem \ref{thm:mn-congs}.

% END OF PROOF

We will state one corollary regarding the retractable ideals of $\Mot_n$.
...
\begin{table}[ht]
  ...
  \begin{tabular}{| c | c | c |}
    ...
    \hline
    $\Sym_n$ & $n!$ & $3$ \\
    $\I_n$ & $\sum_{k=0}^n \binom{n}{k} \frac{n!}{(n-k)!}$ & $3n - 1$ \\
...
but $K_4$ will only be used when $k=4$.  Since $\Alt_2$ is trivial, we have
$R_{I_1,\Alt_2} = R_1$.  Note also that, since there is only
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \node at (-6.83, 13) {$=\nabla_{\Jon_n}$};
    \node at (-6.83,  2) {$=\Delta_{\Jon_n}$};
    \draw (d1)--(l1)--(R1)--(r1)--(d1);
    \draw (R1)--(R3)--(R5);
    \draw[dashed] (R5)--(Rno);
  \end{tikzpicture}
  \caption[Congruence lattice of $\Jon_n$]{Congruence lattice of $\Jon_n$ for $n \geq 4$ when
...
shown in \cite[2.1]{halverson_2014}.
Again we can see that this monoid contains the Motzkin monoid; in fact, it is
clear from the definitions that $\Mot_n = \PP_n \cap \PB_n$.
Its congruence lattice has the same
...
\begin{figure}[p]
  ...
  \begin{tikzpicture}
    ...
    \draw[dashed] (R3)--(Rn);
  \end{tikzpicture}
  \caption[Congruence lattice of $\Prt_n$ (or $\PB_n$)]{Congruence lattice of
    $\Prt_n$ (or $\PB_n$) for $n \geq 3$}
  \label{fig:pn-congs}
\end{figure}
