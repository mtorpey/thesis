...
>                     rec(info:=true)));
\end{verbatim}
  \caption[Congruence lattices of $\Prt_3$ and $\Mot_4$ produced by the
  Semigroups package]{Congruence lattices of $\Prt_3$ (left) and $\Mot_4$
    (right), as produced by the Semigroups package for GAP.  Here `T' represents
    the trivial congruence, `U' the universal congruence, and `R' a Rees
    congruence.  Figures \ref{fig:pn-congs} and \ref{fig:mn-cong-lattice} show
    these lattices with more meaningful labels}
    \label{fig:gap-lattices}
\end{figure}

With the rapidly increasing size of $\Prt_n$ (see Table
...
descriptions \cite[\S7]{ourpaper} (detailed in Theorem \ref{thm:mn-congs}).
...
\begin{figure}[p]
  ...
  \begin{tikzpicture}
    ...
    % Odd
    \node[cong] (Rno) at (-10,19) {$R_n$};
    \node[cong] (RA5) at (-10,16) {$R_{I_3,\Alt_5}$};
    \node[cong] (R3)  at (-10,14) {$R_3$};
    \node[cong] (RS3) at (-10,12) {$R_{I_1,\Sym_3}$};
    \node[cong] (RA3) at (-10,10) {$R_{I_1,\Alt_3}$};
    \node[cong] (R1)  at (-10, 8) {$R_1$};
    \node[cong] (l1)  at (-13, 6) {$\lambda_1$};
    \node[cong] (r1)  at (- 7, 6) {$\rho_1$};
    \node[cong] (d1)  at (-10, 4) {$\delta_1$};
    \node at (-8.83, 19) {$=\nabla_{\Brau_n}$};
    \node at (-8.83,  4) {$=\Delta_{\Brau_n}$};
    \draw (d1)--(l1)--(R1)--(r1)--(d1);
...
even, then the congruence lattice is isomorphic to that of $\Mot_{n/2}$, and its
...
\lambda_{\Sym_2} = \zeta_{I_1,\Sym_2,\LL^0},\quad
\rho_{\Sym_2} = \zeta_{I_1,\Sym_2,\RR^0},\quad
R_{I_1,\Sym_2};$$
...
$k \in \{3, \ldots, n\}$.  These results are proven in \cite[\S5]{ourpaper}, and
the lattice is shown in Figure \ref{fig:pn-congs}.
