...
% \darc56
% \stline21
% \draw(6.4,1)node[right]{$=\alpha\beta$};
% \end{scope}
% %
% \end{tikzpicture}
% \end{center}
% \vspace{-5mm}
% \caption{Two partitions $\alpha,\beta\in\P_6$ (left), the product graph
% $\Pi(\alpha,\beta)$ (middle), and their product $\alpha\beta\in\P_6$ (right).}
% \label{fig:P6}
% \end{figure}
...
The (\emph{co})\emph{domain}, (\emph{co})\emph{kernel} and \emph{rank} of a partition $\alpha=\bipartABCD\in\P_n$ are defined as follows:~
\begin{itemize}
\item
...
\subsection{Other diagram monoids}\label{sec:prelim_other}
...
\node[rounded corners,rectangle,draw,fill=blue!20] (D) at (6,4) {$\Mot_n$};
...
\node[rounded corners,rectangle,draw,fill=green!20] (H) at (3,0) {$\{\id_n\}$};
%
...
%For $a\in S$, we write $J_a$ for the $\gJ$-class of $a$ in $S$.
...
Let $\mathcal{K}_n$ be any of the monoids $\P_n,\PB_n,\B_n,\PP_n,\Mot_n,\I_n,\mathcal J_n,\O_n$.  If $\alpha,\beta\in\mathcal K_n$, then
  \begin{itemize}
\itemit{i} $\alpha\R\beta\iff\dom(\alpha)=\dom(\beta)$ and $\ker(\alpha)=\ker(\beta)$,
\itemit{ii} $\alpha\L\beta\iff\codom(\alpha)=\codom(\beta)$ and $\coker(\alpha)=\coker(\beta)$,
\itemit{iii} $\alpha\gJ\beta\iff\alpha\D\beta\iff\rank(\alpha)=\rank(\beta)$,
\itemit{iv} $J_\alpha\leq J_\beta\iff\rank(\alpha)\leq\rank(\beta)$.
\epfres
\end{itemize}
\end{proposition}
...
%
%Roughly speaking, a retractable ideal can somehow be mapped onto to the minimal ideal and a lifting congruence is a congruence on the minimal ideal that can be used to define a congruence on the whole semigroup.
%
...
%As noted above, we will be especially interested in a particular kind of ideal.
...
%{\red Recall that the minimal ideal $M$ of a finite semigroup $S$ is \emph{simple} and \emph{completely regular}: i.e., $M$ consists of a single $\gJ$-class and is a union of groups \cite[Proposition A.4.5]{RSbook}.  In particular, if $M$ is $\H$-trivial, then $M$ is a rectangular band.  }
...
%since the minimal ideal is completely simple, it may have many congruences; see \cite[Theorem~3.5.9]{Howie}.  However, usually only few of these congruences are lifting congruences.
...
\begin{definition}
\label{Rel-nu}
%For a normal subgroup $N$ of a maximal subgroup of a semigroup~$S$, define the relation
%\[
%\nu_N = S^1(N\times N)S^1\cap(J\times J) = \set{ (sxt,syt)\in J\times J}{ x,y\in N,\ s,t\in S^1},
%\]
%where $J$ is the $\gJ$-class of $S$ containing $N$.
...
as follows.
%If $J$ is a subsemigroup of $S$, then $\nu_N$ is a congruence on $J$.  Otherwise, since
...
%For reasons that will become clear later (see especially Lemma \ref{lem:nu} and Remark \ref{rem:TX}), we will mostly be concerned with $\nu_N$ relations in the case that $J$ is stable and regular.
%
...
\itemit{iv} $\lambda_{\C_1}\cap\mu_{\C_2} = \rho_{\C_1}\cap\mu_{\C_2} = \mu_{\C_1}$,
\itemit{v} $\lambda_{\C_1}\vee\mu_{\C_2} = \lambda_{\C_2}$ and $\rho_{\C_1}\vee\mu_{\C_2} = \rho_{\C_2}$,
...
$(xf,z)\in \lambda_1$ and $(z,y)\in \rho_2$, implying $(x,y)\in\lambda_1\vee\rho_2$. %, and completing the proof.
...
%, which also pictures the $\eta$ relation.
...
%\node[rounded corners,rectangle,draw,fill=blue!20] (e) at (3,-2) {$\eta_{\C}$};
%
\draw
(d)--(l)
(d)--(r)
...
%
%\node[rounded corners,rectangle,draw,fill=blue!20] (e1) at (3,-2) {$\eta_{\C_1}$};
%\node[rounded corners,rectangle,draw,fill=blue!20] (e2) at (0,0) {$\eta_{\C_2}$};
\draw
(d)--(l) (m)--(ml)
(d)--(m) (l)--(ml) (rl)--(mrl)
(d)--(r) (m)--(mr)
...
%$J_q$ and~$I_q$ ($q=0,1,\ldots,n$), respectively, where
%$J_q=J_q(\I_n)=\set{\alpha\in\I_n}{\rank(\alpha)=q}$ and $I_q=I_q(\I_n)=\set{\alpha\in\I_n}{\rank(\alpha)\leq q}$, respectively.
...
%Although the next result is not essential for our main purposes, we record it for completeness.  Here and elsewhere, we use the well-known fact that any rectangular band satisfies the identity $x=xyx$; see for example \cite[Section~4.4]{Howie}.
%
%
%\begin{proposition}\label{prop:retractable_ideals_Pn}
%An ideal $I_q$ of $\P_n$ is retractable if and only if $q\leq1$.
%\end{proposition}
%
%\begin{proof} One direction follows from Lemma \ref{HatRetract}.  Conversely, suppose $g:I_q\to I_0$ is a retraction for some $q\geq2$.  Now define the partitions $\alpha = \partpermII1212$, $\beta = \emptypartperm$ and $\ga = \partIII{1,2}3\cdots n{1,2}3\cdots n$, noting that $\alpha,\beta,\ga\in I_2\sub I_q$.  From $\beta=\alpha\beta\alpha$ and $\beta\in I_0$, we obtain $\beta=\beta g=(\alpha\beta\alpha)g=(\alpha g)(\beta g)(\alpha g)=\alpha g$.  But also $\ga=\alpha\ga\alpha$ with $\ga\in I_0$, and we deduce $\ga=\alpha g$ in similar fashion, contradicting the fact that $\beta\not=\ga$. \end{proof}
...
\begin{proof} Let $\Om=\set{ (\alpha,\beta)\in J_q\times J_q}{\alpha\H \beta,\
...
\begin{proof} To prove (i), suppose $\Rb_1\sub\xi$.  Let $\alpha\in\P_n$ with $\rank(\alpha)\leq1$.
...
%These are the non-trivial join-irreducible lower congruences; see Figure \ref{fig-CongPn}.
...
\begin{itemize}\begin{multicols}{2}
\itemit{i} $\rho_0=\cg{\alpha}{\beta}\iff (\alpha,\beta)\in\rho_0\setminus\Delta$,
\itemit{ii} $\lambda_0=\cg{\alpha}{\beta}\iff (\alpha,\beta)\in\lambda_0\setminus\Delta$,
\itemit{iii} $\mu_1=\cg{\alpha}{\beta}\iff (\alpha,\beta)\in\mu_1\setminus\Delta$,
\itemit{iv} $\mu_{\S_2}=\cg{\alpha}{\beta}\iff (\alpha,\beta)\in\mu_{\S_2}\setminus\mu_1$.
\end{multicols}\end{itemize}
\end{proposition}

\begin{proof} The ``only if'' parts are clear for each statement.
%
...
\begin{proposition}\label{prop:joins2}
...
\begin{proof} Again, the ``only if'' parts are obvious.  Beginning with (i), suppose $(\alpha,\beta)\in R_0\sm(\rho_0\cup\lambda_0)$, and write $\alpha=\partII{A_1}\cdots{A_r}{B_1}\cdots{B_s}$ and $\beta=\partII{C_1}\cdots{C_t}{D_1}\cdots{D_u}$,
%
...
%the results proved above.  We give the details for $\rho_{\S_2}$, and leave the reader to check the others.  Suppose $(\alpha,\beta)\in\rho_{\S_2}\setminus \rho_1$ and $(\ga,\de)\in \rho_{\S_2}\setminus\mu_{\S_2}$, and write $\xi=\cgset\alpha\beta\ga\de$.  Clearly $\xi\sub\rho_{\S_2}$.  From $\rho_{\S_2}\setminus\rho_1=\mu_{\S_2}\setminus\mu_1$, Proposition~\ref{prop:small_congruences}(iv) gives $\mu_{\S_2}=\cg\alpha\beta\sub\xi$.  From $\rho_{\S_2}\setminus\mu_{\S_2}=\rho_1\setminus\mu_1=(\rho_0\setminus\Delta)\cup(\rho_1\sm(\mu_1\cup\rho_0))$, Propositions \ref{prop:small_congruences}(i) and \ref{prop:joins2}(ii) give $\cg\ga\de=\rho_0$ or $\rho_1$; in either case, we have $\rho_0\sub\xi$.  But then $\rho_{\S_2}=\mu_{\S_2}\vee\rho_0\sub\xi$.
\end{rem}
...
\bigskip\noindent {\bf Subcase 2.2.}  Finally, suppose $i,j\in C_1$.  For each $1\leq p\leq q$, choose some $a_p\in A_p$.  Write $\bn\setminus\{a_1,\ldots,a_q\}=\{x_1,\ldots,x_{n-q}\}$, assuming that $x_1=i$ and $x_2=j$, and put
$\ga =
\Big(
...
\itemit{iii} $(\alpha,\beta)=(\ga_2\alpha_1\de_2,\ga_2\beta_1\de_2)$.
\end{multicols}\end{itemize}
\end{lemma}
...
\bigskip\noindent {\bf Case 2.}  Now suppose $\xi=R_N$ for some non-trivial normal subgroup $N$ of $\S_q$, where $q\geq3$.  Now, $R_{q-1}=\Rb_{q-1}^\sharp\sub\xib^\sharp$ by Case 1, so it remains to show that $R_N\setminus R_{q-1}\sub\xib^\sharp$.
...
\section{The partial Brauer monoid \boldmath{$\PB_n$}}\label{sec:PBn}
...
\section{The planar partition monoid \boldmath{$\PP_n$} and the Motzkin monoid \boldmath{$\Mot_n$}}
\label{sec:PPnMn}
...
\begin{itemize}
\itemit{i} The ideals of $\PP_n$ (resp., $\Mot_n$) are the sets $I_q$, for $q=0,\ldots,n$, yielding the Rees congruences $R_q=R_{I_q}$, as in Definition \ref{defn:Rees}.
\itemit{ii} The retractable IN-pairs of $\PP_n$ (resp., $\Mot_n$) are $\C_0=(I_0,\{\overline{\id}_1\})$ and $\C_1=(I_1,\{\overline{\id}_2\})$, yielding the congruences $\lambda_0$, $\rho_0$, $\mu_0$, $\lambda_1$, $\rho_1$, $\mu_1$, respectively, as in Definition \ref{defn:lrmC}.
\itemit{iii} The above congruences are distinct, and they exhaust all the congruences of $\PP_n$ (resp., $\Mot_n$).
\itemit{iv} The $\ast$-congruences of $\PP_n$ (resp., $\Mot_n$) are the same, but with $\lambda_0,\lambda_1,\rho_0,\rho_1$ excluded.
\itemit{v} The Hasse diagrams of the congruence and $*$-congruence lattices of $\PP_n$ (resp., $\Mot_n$) are shown in Figure~\ref{fig-CongPPn}.
\end{itemize}
\end{thm}
...
\vspace{-3mm}
\caption{
Hasse diagrams of $\Cong(\PP_n)$ and $\Cong(\Mot_n)$ (left), and of
$\Cong^\ast(\PP_n)$ and $\Cong^\ast(\Mot_n)$ (right).
...
\begin{proposition}[cf.~Proposition \ref{prop:small_congruences}]
...
%
\begin{proposition}[cf.~Proposition \ref{prop:joins2}]
...
\begin{proposition}[cf.~Proposition \ref{prop-aa3}]
\label{prop-aa3:PPn}
...
\begin{definition}
\label{defn:retr_Bn}
For a Brauer element $\alpha$ of rank $2$ with transversals $\{i,j'\}$ and $\{k,l'\}$, let $\alphah$
be the Brauer element of rank $0$ obtained by replacing these transversals by the non-transversals $\{i,k\}$ and $\{j',l'\}$.  For $\alpha\in\B_n$ with $\rank(\alpha)=0$, let $\alphah=\alpha$.
\end{definition}

We will make no use of the mapping from Definition \ref{defn:hat} in this section, so there should be no confusion arising from our re-use of the $\alphah$ notation.  An example calculation is given in Figure \ref{fig:beh}.
...
\darcx25{.5}
\darcx16{.8}
\draw(6.4,1)node[right]{$=\alphah$};
\end{scope}
%
\end{tikzpicture}
\end{center}
\vspace{-5mm}
\caption{A rank-$2$ Brauer element $\alpha\in\B_6$ (left), with $\alphah\in\B_6$ (right).}
\label{fig:beh}
\end{figure}
...
\begin{lemma}
\label{Bretract}
For $n$ even, the map $f: I_2\rightarrow I_0:\alpha\mt\alphah$ is a retraction.
\end{lemma}
...
\itemit{vii} The $\ast$-congruences of $\B_n$ are the same, but with $\lambda_1$, $\rho_1$ ($n$ odd) or $\lambda_0$, $\lambda_{\S_2}$, $\lambda_2$, $\lambda_K$, $\rho_0$, $\rho_{\S_2}$, $\rho_2$, $\rho_K$ ($n$~even) excluded.
\end{itemize}
\end{thm}
...
\begin{proposition}\label{prop:rl_Bn}
The relations $\rho_z$ and $\lambda_z$ are principal congruences on $\B_n$.  Moreover, if $\alpha,\beta\in\B_n$, then
\begin{itemize}\begin{multicols}{2}
\itemit{i} $\rho_z=\cg\alpha\beta\iff (\alpha,\beta)\in\rho_z\setminus\Delta$,
\itemit{ii} $\lambda_z=\cg\alpha\beta\iff (\alpha,\beta)\in\lambda_z\setminus\Delta$.
\end{multicols}\end{itemize}
\end{proposition}
...
\begin{itemize}\begin{multicols}{2}
\itemit{i} $\mu_{\S_2}=\cg\alpha\beta\iff (\alpha,\beta)\in\mu_{\S_2}\setminus\Delta$,
\itemit{ii} $\mu_2=\cg\alpha\beta\iff (\alpha,\beta)\in\mu_2\setminus\mu_{\S_2}$,
\itemit{iii} $\mu_K=\cg\alpha\beta\iff (\alpha,\beta)\in\mu_K\setminus\mu_2$.
\end{multicols}\end{itemize}
\end{proposition}
...
Finally, for (iii), let $(\alpha,\beta)\in \mu_K\setminus\mu_2$ and write $\xi=\cg\alpha\beta$.
Since $(\alpha,\beta)\not\in\mu_2$,
it follows that $\rank(\alpha)=\rank(\beta)=4$, $\alpha\H\beta$ and $\phi(\alpha,\beta)\in K\setminus\{\id_4\}$.
...
$(\si_1\alpha,\si_1\beta)\in\mu_2\setminus\mu_{\S_2}$, so part (ii) gives $\mu_2=\cg{\si_1\alpha}{\si_1\beta}\sub\xi$.
%
It remains to show that $\mu_K\setminus\mu_2\sub\xi$, so let $(\ga,\de)\in\mu_K\setminus\mu_2$ be arbitrary.  As above, we may write
$
\ga = \Big(
{ \scriptsize \renewcommand*{\arraystretch}{1}
\begin{array} {\c|\c|\c|\c|\c|\c|\cend}
...
\hspace{-1.5 truemm} \Big)$.  Then $(\ga,\de)=(\si_2\alpha\si_3,\si_2\beta\si_3)\in\xi$, as required.~\end{proof}

The remaining principal lower congruences are the $\lam,\rho,R$ congruences arising from the pair $\C_2$.

\newpage

\begin{proposition}\label{prop:joins2_Bn}
Let $n=2m\geq4$ be even.  The congruences
$\rho_2$, $\lambda_2$ and $R_2$ are all principal.  Moreover, if $\alpha,\beta\in\B_n$, then
\begin{itemize}\begin{multicols}{2}
\itemit{i} $\rho_2=\cg\alpha\beta\iff (\alpha,\beta)\in\rho_2\sm(\mu_2\cup\rho_{\S_2})$,
\itemit{ii} $\lambda_2=\cg\alpha\beta\iff (\alpha,\beta)\in\lambda_2\sm(\mu_2\cup\lambda_{\S_2})$,
\itemit{iii} $R_2=\cg\alpha\beta\iff (\alpha,\beta)\in R_2 \sm
(\lambda_2\cup\rho_2\cup R_{\S_2})$.
\end{multicols}
\end{itemize}
\end{proposition}

\begin{proof} Again, it suffices prove to the direct inclusion ($\subseteq$) in the ``if'' statement in each case.
...
\bigskip\noindent {\bf Case 1.}  Suppose first that $\dom(\alpha)=\dom(\beta)$.  Renaming if necessary, we may assume that $A_1$ is not a block of $\beta$.  Write $A_1=\{i,j\}$.  Since $i,j\in\bn\setminus\dom(\alpha)=\bn\setminus\dom(\beta)$, we may assume that $C_1=\{i,k\}$ and $C_2=\{j,l\}$ for some $k,l$.  Define $\si=\Big(
{ \scriptsize \renewcommand*{\arraystretch}{1}
\begin{array} {\c|\c|\c|\c|\c|\c|\c|\c|\c|\cend}
...
\begin{proof} By Lemma \ref{lem:notH_prelim_Bn}, there exists $(\ga,\de)\in\xi$ with $\rank(\ga)=q$ and $\rank(\de)<q$.  It then follows from Lemma \ref{lem:qp_Bn} that $R_q\sub\xi$. \end{proof}
...
\begin{proof} Write $\alpha=\partI{a_1}{a_q}{A_1}{A_r}{b_1}{b_q}{B_1}{B_r}$ and $\beta=\partI{a_1}{a_q}{A_1}{A_r}{b_{1\si}}{b_{q\si}}{B_1}{B_r}$, where $a_1<\cdots<a_q$, noting that $\phi(\alpha,\beta)=\si^{-1}$, so $N=\normal\si$.  Put $\ga_1=\partI1q{q+1,q+2}{n-1,n}{a_1}{a_q}{A_1}{A_r}$ and $\ga_2=\partI{b_1}{b_q}{B_1}{B_r}1q{q+1,q+2}{n-1,n}$, noting that $\ve_q=\ga_1\alpha\ga_2\mathrel\xi\ga_1\beta\ga_2=\sib$.
%
Define
\[
\de_1=\Big(
...
\bigskip\noindent {\bf Case 1.}  Suppose first that $\xi=R_q$ for some $q\geq3$.  So $(\alpha,\beta)\in R_q\setminus R_{\S_q}$.  Thus, renaming $\alpha,\beta$ if necessary, we have $\rank(\alpha)=q$, and either $\rank(\beta)<q$ or else $\rank(\beta)=q$ and $(\alpha,\beta)\not\in{\H}$.  It then follows from Lemma \ref{lem:qp_Bn} or \ref{lem:notH_Bn}, respectively, that $\xi=R_q\sub\zeta$.
...
\section{The Jones monoid \boldmath{$\J_n$}}\label{sec:Jn}
...
\itemit{vi} The $\ast$-congruences of $\J_n$ are the same, but with $\lambda_1$, $\rho_1$ ($n$ odd) or $\lambda_0$, $\lambda_2$, $\rho_0$, $\rho_2$ ($n$ even) excluded.
\end{itemize}
\end{thm}
...
\begin{proposition}[cf.~Proposition \ref{prop:small_congruences:PPn}]
\label{prop:rlJeven}
...
\begin{proof} Write ${\mathrel\xi}=\cg\alpha\beta$ and $\si=\partIV
...
\begin{itemize}\begin{multicols}{2}
\itemit{i} $\rho_1=\cg\alpha\beta\iff (\alpha,\beta)\in\rho_1\setminus\Delta$,
\itemit{ii} $\lambda_1=\cg\alpha\beta\iff (\alpha,\beta)\in\lambda_1\setminus\Delta$.
\end{multicols}\end{itemize}
\end{proposition}
...
\begin{proof} Write $\alpha=\partI{a_1}{a_q}{A_1}{A_r}{a_1}{a_q}{A_1}{A_r}$, where $a_1<\cdots<a_q$.  There are three possibilities:
\begin{itemize}
\begin{multicols}{3}
...
\node[black] () at (2,1) {${\Rect\setminus\Rect_1}$};
...
\draw[|-|] (-2,20)--(-2,10);
\draw[-|] (-2,10)--(-2,0);
\draw(-2,5)node[left]{$\beta$};
\draw(-2,15)node[left]{$\alpha$};
\draw(32,5)node[right]{$\phantom{\beta}$};
\draw(32,15)node[right]{$\phantom{\alpha}$};
%
\end{tikzpicture}
}
\end{center}
\vspace{-5mm}
\caption{The curve $\C=\C_1\cup\C_2$ from Case 4 of the proof of Lemma \ref{lem:proj2_Jn}.  The curves $\C_1$ and $\C_2$ are drawn red and blue, respectively.  The regions $\Rect_1^+$ and $\Rect_1^-$ are shaded green and orange, respectively, and the region $\Rect\setminus\Rect_1$ is shaded gray.  Odd-labelled vertices are drawn black, and even-labelled vertices white. }
\label{fig:curve}
\end{figure}
