...
\begin{itemize}\begin{multicols}{2}
\itemit{i} $\rho_0=\cg{\alpha}{\beta}\iff (\alpha,\beta)\in\rho_0\sm\De$,
\itemit{ii} $\lam_0=\cg{\alpha}{\beta}\iff (\alpha,\beta)\in\lam_0\sm\De$,
\itemit{iii} $\mu_1=\cg{\alpha}{\beta}\iff (\alpha,\beta)\in\mu_1\sm\De$, 
\itemit{iv} $\mu_{\S_2}=\cg{\alpha}{\beta}\iff (\alpha,\beta)\in\mu_{\S_2}\sm\mu_1$.
\end{multicols}\end{itemize}
\end{proposition}
...
%the results proved above.  We give the details for $\rho_{\S_2}$, and leave the reader to check the others.  Suppose $(\alpha,\beta)\in\rho_{\S_2}\sm \rho_1$ and $(\ga,\de)\in \rho_{\S_2}\sm\mu_{\S_2}$, and write $\xi=\cgset\alpha\beta\ga\de$.  Clearly $\xi\sub\rho_{\S_2}$.  From $\rho_{\S_2}\sm\rho_1=\mu_{\S_2}\sm\mu_1$, Proposition~\ref{prop:small_congruences}(iv) gives $\mu_{\S_2}=\cg\alpha\beta\sub\xi$.  From $\rho_{\S_2}\sm\mu_{\S_2}=\rho_1\sm\mu_1=(\rho_0\sm\De)\cup(\rho_1\sm(\mu_1\cup\rho_0))$, Propositions \ref{prop:small_congruences}(i) and \ref{prop:joins2}(ii) give $\cg\ga\de=\rho_0$ or $\rho_1$; in either case, we have $\rho_0\sub\xi$.  But then $\rho_{\S_2}=\mu_{\S_2}\vee\rho_0\sub\xi$.
\end{rem}
...
\begin{proof} The ``only if'' part being clear, suppose $(\alpha,\beta)\in\xi\sm\xi^-$.  We must show that $\xi\sub\cg\alpha\beta$.  In fact, by Lemma \ref{lemma-aa2}, it is enough to show that $\xib\sub\cg\alpha\beta$.
...
\begin{itemize}\begin{multicols}{2}
\itemit{i} $\rho_z=\cg\alpha\beta\iff (\alpha,\beta)\in\rho_z\sm\De$, 
\itemit{ii} $\lam_z=\cg\alpha\beta\iff (\alpha,\beta)\in\lam_z\sm\De$.
\end{multicols}\end{itemize}
\end{proposition}
...
\begin{itemize}\begin{multicols}{2}
\itemit{i} $\mu_{\S_2}=\cg\alpha\beta\iff (\alpha,\beta)\in\mu_{\S_2}\sm\De$,
\itemit{ii} $\mu_2=\cg\alpha\beta\iff (\alpha,\beta)\in\mu_2\sm\mu_{\S_2}$, 
\itemit{iii} $\mu_K=\cg\alpha\beta\iff (\alpha,\beta)\in\mu_K\sm\mu_2$.
\end{multicols}\end{itemize}
\end{proposition}
...
Since $(\alpha,\beta)\not\in\mu_2$, 
it follows that $\rank(\alpha)=\rank(\beta)=4$, $\alpha\H\beta$ and $\phi(\alpha,\beta)\in K\sm\{\id_4\}$.  
...
$(\si_1\alpha,\si_1\beta)\in\mu_2\sm\mu_{\S_2}$, so part (ii) gives $\mu_2=\cg{\si_1\alpha}{\si_1\beta}\sub\xi$.  
%
...
\itemit{ii} $\lam_2=\cg\alpha\beta\iff (\alpha,\beta)\in\lambda_2\sm(\mu_2\cup\lam_{\S_2})$,
...
\bigskip\noindent {\bf Case 1.}  Suppose first that $\dom(\alpha)=\dom(\beta)$.  Renaming if necessary, we may assume that $A_1$ is not a block of $\beta$.  Write $A_1=\{i,j\}$.  Since $i,j\in\bn\sm\dom(\alpha)=\bn\sm\dom(\beta)$, we may assume that $C_1=\{i,k\}$ and $C_2=\{j,l\}$ for some $k,l$.  Define $\si=\Big( 
{ \scriptsize \renewcommand*{\arraystretch}{1}
\begin{array} {\c|\c|\c|\c|\c|\c|\c|\c|\c|\cend}
...
\bigskip\noindent {\bf Case 1.}  Suppose first that $\xi=R_q$ for some $q\geq3$.  So $(\alpha,\beta)\in R_q\sm R_{\S_q}$.  Thus, renaming $\alpha,\beta$ if necessary, we have $\rank(\alpha)=q$, and either $\rank(\beta)<q$ or else $\rank(\beta)=q$ and $(\alpha,\beta)\not\in{\H}$.  It then follows from Lemma \ref{lem:qp_Bn} or \ref{lem:notH_Bn}, respectively, that $\xi=R_q\sub\zeta$.
...
\begin{itemize}\begin{multicols}{2}
\itemit{i} $\rho_0=\cg\alpha\beta\iff (\alpha,\beta)\in\rho_0\sm\Delta$, 
\itemit{ii} $\lambda_0=\cg\alpha\beta\iff (\alpha,\beta)\in\lambda_0\sm\Delta$.
\epfres
\end{multicols}\end{itemize}
\end{proposition}
...
\begin{itemize}\begin{multicols}{2}
\itemit{i} $\rho_1=\cg\alpha\beta\iff (\alpha,\beta)\in\rho_1\sm\Delta$,
\itemit{ii} $\lambda_1=\cg\alpha\beta\iff (\alpha,\beta)\in\lambda_1\sm\Delta$.
\end{multicols}\end{itemize}
\end{proposition}
