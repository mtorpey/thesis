...
\begin{itemize}
\item $\alpha ~\mathcal{L}~ \beta$ if and only if $\im \alpha = \im \beta$,
\item $\alpha ~\mathcal{R}~ \beta$ if and only if $\ker \alpha = \ker \beta$,\cite[9.2]{ruskuc-notes}
\item $\alpha ~\mathcal{D}~ \beta$ if and only if $\rank \alpha = \rank \beta$.\cite[10.6]{ruskuc-notes}
\end{itemize}

This last allows us to name the semigroup's $\mathcal{D}$-classes $D_1, D_2,
\dots, D_n$, where each $D_k$ is the $\mathcal{D}$-class consisting of
transformations with rank $k$.  The usual partial ordering of
$\mathcal{J}$-classes \cite[p.47]{howie} (which in a finite semigroup are the
same as $\mathcal{D}$-classes) gives $D_1 < D_2 < \dots < D_n$.

Inside a given $\mathcal{D}$-class $D_k$, elements are divided into
$\mathcal{L}$-classes according to their image set; since all elements have rank
$k$, their images must have size $k$, and so there are $\binom{n}{k}$
$\mathcal{L}$-classes in total.  Similarly, the elements of $D_k$ are divided
into $\mathcal{R}$-classes according to their kernel; there is one class for
each possible $k$-partition of an $n$-set, so there are the Stirling number of
the second kind, $S(n,k)$ $\mathcal{R}$-classes in total.

Each $\mathcal{H}$-class in $D_k$ is the intersection of an $\mathcal{L}$-class
and an $\mathcal{R}$-class, so each one corresponds to an image-kernel pair
(hence we will talk about the \textit{image} and \textit{kernel of an $\mathcal{H}$-class}).
...
\section{Group $\mathcal{H}$-classes}
\label{sec:grouphclasses}
To determine which $\mathcal{H}$-classes of $D_k$ are groups, we recall that in any
semigroup an $\mathcal{H}$-class H is a group if and only if it contains an idempotent
...
Hence an $\mathcal{H}$-class of $D_k$ is a group if and only if its image is a transversal
of its kernel.
...
\leavevmode
  \begin{enumerate}
  \item For any two distinct $\mathcal{L}$-classes $L_1$ and $L_2$ of $D_k$ (for
    $1 < k \leq n$) there is an $\mathcal{R}$-class $R$ such that $L_1 \cap R$
    is a group $\mathcal{H}$-class, but $L_2 \cap R$ is not.
  \item For any two distinct $\mathcal{R}$-classes $R_1$ and $R_2$ of $D_k$ (for
    $1 \leq k \leq n$) there is an $\mathcal{L}$-class $L$ such that $L \cap
    R_1$ is a group $\mathcal{H}$-class, but $L \cap R_2$ is not.
  \end{enumerate}
  \begin{proof}
    \leavevmode
    \begin{enumerate}
    \item Let $L_1$ and $L_2$ be distinct $\mathcal{L}$-classes of $D_k$, with
      ...
      $P$.  Let $R$ be the $\mathcal{R}$-class with kernel $P$, and we have that
      $L_1 \cap R$ is a group $\mathcal{H}$-class but $L_2 \cap R$ is not.

    \item Let $R_1$ and $R_2$ be distinct $\mathcal{R}$-classes of $D_k$.  These two classes
      ...
      transversal of $P_2$.  Hence, if $L$ is the $\mathcal{L}$-class corresponding to image
      $X$, $L \cap R_1$ is a group $\mathcal{H}$-class but $L \cap R_2$ is not a group
      $\mathcal{H}$-class.
    \end{enumerate}
  \end{proof}
\end{lemma}
...
The \textbf{principal factor} $D_k^*$ of a $\mathcal{D}$-class $D_k$ is the Rees
...
The principal factor of a $\mathcal{D}$-class must always be a 0-simple
...
The rows and columns of the matrix $P$ correspond respectively to the $\mathcal{L}$-classes and
$\mathcal{R}$-classes of $D_k$, and $G$ is the group isomorphic to each of $D_k$'s group
$\mathcal{H}$-classes.  Since the elements of an $\mathcal{H}$-class here correspond to all the permutations
...
corresponding $\mathcal{H}$-class is a group.  By Lemma \ref{lem:hclasses} we can see that
...
\label{sec:k1}
First let us consider the linked triples of $D_1^*$.  Since this $\mathcal{D}$-class
...
in its own $\mathcal{H}$-class.  Hence each $\mathcal{H}$-class is a group, so the matrix $P$ has no
...
$\mathcal{H}$-class contains just one element.
...
For brevity, we let $u_n = \binom{n}{k} \cdot S(n,k)$, the number of
$\mathcal{H}$-classes in $D_k$.
...
%   \centering
  \begin{tabular}{| r | r | c | r |}
    ...
    \hline
    $1$ & $[1, \Delta_I, \mathcal{T}] (\forall \mathcal{T} \subseteq \Lambda \times \Lambda), D_1^* \times D_1^*$ & $B_n + 1$
    & $|\Lambda/\mathcal{T}|+1 (\forall \mathcal{T} \subseteq \Lambda \times \Lambda)$, $1$ \\
    $2$ & $[1, \Delta, \Delta], [S_2, \Delta, \Delta], D_2^* \times D_2^*$ & $3$ & $2u_2+1, u_2+1, 1$ \\
    $3$ & $[1, \Delta, \Delta], [A_3, \Delta, \Delta], [S_3, \Delta, \Delta], D_3^* \times D_3^*$ & $4$ & $6u_3+1, 2u_3+1, u_3+1, 1$ \\
    $4$ & $[1, \Delta, \Delta], [V_4, \Delta, \Delta], [A_4, \Delta, \Delta], [S_4, \Delta, \Delta], D_4^* \times D_4^*$ & $5$ & $24u_4+1, 6u_4+1, 2u_4+1, u_4+1, 1$ \\
    $\geq 5$ & $[1, \Delta, \Delta], [A_k, \Delta, \Delta], [S_k, \Delta, \Delta], D_k^* \times D_k^*$ & $4$ & $k!u_k+1, 2u_k+1, u_k+1, 1$ \\
    \hline
  \end{tabular}
% \end{table}
\end{theorem}
